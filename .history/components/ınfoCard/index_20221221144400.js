...
import { BidModal, LoadingModal, SharesOnSale } from "../detail";
...
export default function InfoCard({ horse, saleInfo }) {
  ...
  for (let i = 0; i < horse?.auctionInfo.length; i++) {
    const auctioner = objToArr.filter(
      (auctioner) =>
        auctioner.publicAddress === horse?.auctionInfo[i]?.sellerAddress
    );
  }
  console.log("auct", auctioner);
    ...
    <div className="w-full">
      ...
      <div className={styles.userCard}>
        ...
        <div className={styles.userCardInfo}>
          ...
          <span>P/S {horse?.totalAmount} </span>
        </div>
        <Link href={horse?.preferenceDescription}>
          <img
            src="/icons/logo-tjk.png"
            alt="Picture of the author"
            width={70}
            height={70}
            className={styles["tjk"]}
          />
        </Link>
      </div>
