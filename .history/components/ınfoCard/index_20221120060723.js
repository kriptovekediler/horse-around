...
export default function InfoCard({ horse, saleInfo }) {
  ...
  const buyItem = async () => {
    ...
    await axios
      .post("https://horse-around-backend.herokuapp.com/buy_horse", {
      ...
      <div className={styles.userCard}>
        ...
        <div className={styles.userCardInfo}>
          {horse?.status === 3 ? (
            ...
            <span className={styles.badge}>
              {web3?.utils?.fromWei(saleInfo?.price.toString(), "ether")} ETH
            </span>
          ) : (
            <p></p>
          )}
          <span>P/S {horse?.saleInfo[horse?.saleInfo.length - 1]?.ps}</span>
        </div>
      </div>
