...
export default function InfoCard({ horse, saleInfo }) {
  ...
  useEffect(() => {
    (horse?.auctionInfo[horse?.auctionInfo.length-1]?.startingDate + horse?.auctionInfo[horse?.auctionInfo.length-1]?.duration)*1000 > Date.now() ? setStatus(true) : setStatus(false);
    const date = new Date((horse?.auctionInfo[horse?.auctionInfo.length-1]?.deadline*1000));
  ...
  const buyItem = async() => {
    ...
    console.log(horse.saleInfo.nonce, horse?.saleInfo.signature, horse?.publicAddress, horse?.horseId)

    const buyItemTx = await contractMarketplace.methods.buy(
      horse.publicAddress,
      horse.horseId,
    ...
    await axios.post("https://horse-around-backend.herokuapp.com/buy_horse",{
    ...
    <LoadingModal
      ...
      {horse?.status === 4 && (
            ...
            <div>
              {status ? (
                <>
                  <Clock startingDate={horse?.auctionInfo[horse?.auctionInfo.length-1]?.startingDate} date={(horse?.auctionInfo[horse?.auctionInfo.length-1]?.startingDate + horse?.auctionInfo[horse?.auctionInfo.length-1]?.duration)*1000} small />
                  <span className="text-sm 2xl:text-xl">
                  Opening Bid: {web3?.utils?.fromWei(horse?.auctionInfo[horse?.auctionInfo.length-1]?.openingBid,'ether')} ETH - $ {web3?.utils?.fromWei(horse?.auctionInfo[horse?.auctionInfo.length-1]?.openingBid,'ether') * 1500}
                </span>
              </>
              ) : (
            ...
            <div className={styles.darkBox}>
              ...
              <Space>
                <span className={styles.badge}>{web3?.utils?.fromWei(horse?.auctionInfo[horse?.auctionInfo.length-1]?.reservedPrice,'ether')} ETH</span>
                <span className="text-base 2xl:text-xl text-[#734B1A]">
                {web3?.utils?.fromWei(horse?.auctionInfo[horse?.auctionInfo.length-1]?.reservedPrice,'ether') * 1500} $
                </span>
              </Space>
            </div>
          </Space>
      ...
      <div className={styles.userCard}>
        ...
        <div className={styles.userCardInfo}>
            {horse?.status === 3 ? (
              <span className={styles.badge}>{web3?.utils?.fromWei(horse?.saleInfo[horse?.saleInfo.length-1].price,'ether')} ETH</span>  
            ) : horse?.status === 3 && saleInfo ? (
              <span className={styles.badge}>{web3?.utils?.fromWei(saleInfo?.price.toString(),'ether')} ETH</span>  
            ) : <p></p>} 
            <span>P/S %{horse?.saleInfo[horse?.saleInfo.length-1]?.ps / 100}</span>
        </div>
      </div>
