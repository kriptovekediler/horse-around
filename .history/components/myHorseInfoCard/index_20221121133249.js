...
export default function MyHorseInfoCard({ horse }) {
  ...
  let lastIndexHistory = horse?.saleHistory?.length - 1;
  let lastIndexSale = horse?.saleInfo?.length - 1;
  console.log(horse?.saleInfo[lastIndex]?.ps);
      ...
      <div className={styles.userCard}>
        ...
        <div>
          ...
          <h4 className={styles.horseName} style={{ textAlign: "right" }}>
            {horse?.saleInfo[lastIndexSale]?.ps -
              horse?.saleHistory[lastIndexHistory]?.ps}
          </h4>
        </div>
      </div>
