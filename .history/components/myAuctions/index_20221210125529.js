...
export default function MyAuctions({ account, horses }) {
  ...
  const claim = async (e, bid) => {
    ...
    await axios
      ...
      .then((res) => {
        if (
          res.data.detail.horse.auctionInfo[bid.auctionId].highestBidder ===
          account
        ) {
          alert("You cannot claim your bid because you are the highest bidder");
        } else {
          contractAuction.methods
            .cancelBid(bid.horseId)
            .send({
              from: account,
            })
            .then((res) => {
              ...
              axios
                .post("http://127.0.0.1:8000/cancel_a_bid", {
                  publicAddress: account,
                  horseId: bid?.horseId,
  ...
  const endAuction = async (e, bid) => {
    ...
    await contractAuction.methods
      ...
      .then((res) => {
        console.log("contract res:", res);
        axios
          .post("http://127.0.0.1:8000/end_auction", {
            buyer: account,
    ...
    <table class="table-fixed w-full ">
      <thead>
        <tr className="h-20">
          ...
          {user?.myBids.length > 0 && <th className="text-center">Claim</th>}
        </tr>
      </thead>
      <tbody>
          ...
          return (
            <tr key={i}>
                        ...
                        <Link href={`/owner/${bid.sellerAddress}`}>
                          <a>
                            <span className="ml-4 inline-block">
                              @{bid?.sellerAddress.slice(0, 5)}...
                              {bid?.sellerAddress.slice(-3)}
                            </span>
                          </a>
                        </Link>
                      </span>
              ...
              <td className="text-center">
                {web3?.utils?.fromWei(bid?.bidInfo.bidAmount, "ether") + " "}ETH
              </td>
              {!bid?.isClaimed && (
                <td className="text-center mx-auto">
                  {Object.entries(horses?.detail).map(([key, value]) => {
                    if (
                      value.auctionInfo[bid.auctionId]?.deadline + 7 * 60 <
                        Math.floor(Date.now() / 1000) &&
                      value.auctionInfo[bid.auctionId]?.highestBidder ===
                        account &&
                      bid?.horseId === value.horseId
                    ) {
                      return (
                        <button
                          className="bg-[#39250B] hover:opacity-75 text-white px-4 py-2 rounded-lg"
                          onClick={(e) => endAuction(e, bid)}
                        >
                          End Auction
                        </button>
                      );
