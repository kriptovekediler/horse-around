...
export default function CardItem({ name, image, ownerName, id, price, ownerAddress }) {
  const { web3 } = useWeb3();
  return (
    <div>
        ...
        <div className={styles.cardItemImageBox}>
          ...
          <div className={styles.cardHorseInfo}>
            <div>
              ...
              <span className={styles.horsePrice}>{web3?.utils?.fromWei(price,'ether')} ETH or {1500*web3?.utils?.fromWei(price,'ether')} $ </span>
            </div>
          </div>
        </div>
        </a>
