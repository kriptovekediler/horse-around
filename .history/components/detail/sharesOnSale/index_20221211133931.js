...
export default function SharesOnSale({ horse }) {
  ...
  console.log("sellerInfo", horse?.saleInfo[lastIndex]?.sellerAddress);
  ...
  console.log("Sale Info:", horse?.saleInfo[0]?.sellerAddress * leftAmount);
  ...
  const buyItem = async (e, id) => {
    ...
    setId(id);
    console.log("PriceV2", horse.saleInfo[0].price);
    ...
    const contract = new ethers.Contract(
      "0x670E3D036Ce7147A38194Dfb75165d32Be2e68C8",
      marketplaceNFT,
      wssSigner
    );
    await contract.buyItem(
      ...
      horse?.saleInfo[id]?.sellerAddress,
      { value: (horse.saleInfo[0].price * leftAmount).toString() }
    );
    ...
    await axios
      .post("http://127.0.0.1:8000/buy_horse", {
