...
export default function SharesOnSale({ horse, setSharesModal }) {
  ...
  useEffect(() => {
    ...
    const filteredHorse = objToArrHorse?.map((horse) => {
      return horse.saleInfo;
    });
    data.push(filteredHorse);
    const filterSaleInfo = filteredHorse.map((sale) => sale.saleInfo);
    console.log("saleInfoooo", filteredHorse);
    const filteredUser = objToArr?.map((user) => user.publicAddress);
  }, [users]);
