...
export default function SharesOnSale({ horse, setSharesModal }) {
  ...
  const horseArr = Object.entries(horses);
  horseArr?.map(([key, value]) => {
    const sellerAdress = value.saleInfo.map((sale) => sale.sellerAddress);
    console.log("selleradresss", sellerAdress);
    const filteredUser = userArr.map(([key, value]) => {
      const filter = value.publicAddress;
      console.log("filtereddddddd", filter);
    });
  });
