...
export default function SharesOnSale({ horse, setSharesModal }) {
  ...
  useEffect(() => {
    ...
    function equalsUser(publicAddress, sellerInfo) {
      return publicAddress === sellerInfo;
    }
    console.log("selleraddress", sellerAddress);
    const filteredUser = objToArr?.map((user) => {
      return sellerAddress.map((sel) => {
        if (sel.sellerAddress === user.publicAddress) {
          console.log("dön baba artık", user);
        }
      });
    });
    console.log("dsakljdaskldjlka", filteredUser);
  }, [users]);
