...
export default function SharesOnSale({horse,account}) {
  ...
  for (let i = 0; i < horse.saleInfo.length; i++) {
    data.push({
      ...
      owner: {
        name: "Alexia Cambell",
        userName: `@${horse.saleInfo[i].sellerAddress.slice(0,5)}...${horse.saleInfo[i].sellerAddress.slice(-5)}`,
        avatar: "https://i.pravatar.cc/80",
      },
      ownerPercent: "%100",
      sellingPercent: `%${horse.saleInfo[i].ps / 100}`,
      price: `${web3?.utils?.fromWei(horse?.saleInfo[i].price,'ether') * 1500 } $ OR ${web3?.utils?.fromWei(horse?.saleInfo[i].price,'ether') } ETH`,
      actionButton: {
        id:i,
