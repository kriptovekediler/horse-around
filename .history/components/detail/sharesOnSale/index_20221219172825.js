...
export default function SharesOnSale({ horse, setSharesModal }) {
  ...
  useEffect(() => {
    const getUser = async () => {
      const response = await fetch(
        "https://horse-around-app.herokuapp.com/get_users/"
      );
  ...
  //   console.log("horsidorsi", horse.saleInfo[i]);
  //   setHorseAmount(horse.saleInfo);
  // }
  // console.log("USERS:", users);
  // console.log("Res", resp);
  console.log(users);
  var objToArr = Object.entries(users).map(([key, value]) => value);
  var objToArrHorse = Object.entries(horses).map(([key, value]) => value);
  // console.log("objtoarr", objToArr);
  // console.log("objtoarr222222", objToArrHorse);
  ...
  useEffect(() => {
    const data = [];
    const filteredHorse = objToArrHorse?.map((horse) => {
      return horse;
    });

    const filterSaleInfo = filteredHorse.map((sale) => {
      return sale.saleInfo;
    });
    const sellerAddress = filterSaleInfo.map((i) =>
      i.map((sel) => sel.sellerAddress)
    );
    const onMarket = filterSaleInfo.map((i) =>
      i.map((market) => market.onMarket)
    );

    console.log("Viriviri", onMarket);
    console.log("selleraddress", sellerAddress);
    for (let index = 0; index < sellerAddress.length; index++) {
      const filteredUser = objToArr?.filter(
        (user) => user.publicAddress === sellerAddress[index].sellerAddress
      );
  ...
  for (let i = 0; i < horse.saleInfo.length; i++) {
    var horsesOnMarket = sum(horse?.saleInfo[i]?.onMarket);
  }

  console.log("Fetch", horsesOnMarket);

  console.log("horseeeee", filteredHorse);
  ...
  for (let i = 0; i < horse.saleInfo.length; i++) {
    console.log("horseeee", horse.saleInfo[i].sellerAddress);
    var user = objToArr.filter(
      (user) => user.publicAddress === horse.saleInfo[i].sellerAddress
    );
    console.log("horsee", user);
