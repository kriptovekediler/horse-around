...
export default function SharesOnSale({ horse, setSharesModal }) {
  ...
  useEffect(() => {
    const filteredHorses = objToArrHorse?.map((obj) => {
      const filtered = objToArr?.map((user) => {
        var saleInfo = obj?.saleInfo.filter(
          (i) => i.sellerAddress === user.publicAddress
        );
        console.log("saleeeee", saleInfo);
      });
    });

    setFilteredHorse(filteredHorses);
  }, [users]);
  console.log("filtereddsadasdas", filteredHorse);
