...
export default function SharesOnSale({ horse, account }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (info) => (
        <button className="bg-[#39250B] d-flex text-white hover:bg-goldMetallic transition text-lg sm:text-sm md:text-sn rounded-md h-10 px-2 w-full d-flex items-center justify-between gap-2">
          <>
            <Link href={`/detail/${horse?.horseId}/saleinfo/${info["id"]}`}>
              <a className="d-flex" target={"_self"}>
                <span>Buy Now</span>
