...
export default function SharesOnSale({ horse, setSharesModal }) {
  ...
  console.log("horses", objToArrHorse);
  ...
  useEffect(() => {
    const filteredUser = objToArrHorse?.map((horse) => {
      var currentHorse = horse?.saleInfo.map((info) => {
        var lastFiltered = objToArr?.filter(
          (user) => user.publicAddress === info.sellerAddress
        );
        return {
          ...lastFiltered,
        };
