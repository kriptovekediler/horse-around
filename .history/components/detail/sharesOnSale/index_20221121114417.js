...
export default function SharesOnSale({ horse, account }) {
  ...
  let lastIndex = horse?.saleInfo?.length - 1;
  ...
  const submitHandleAmount = (e) => {
    e.preventDefault();
    if (leftAmount <= horse.saleInfo[lastIndex].ps && leftAmount >= 0) {
      setAmount((prev) => horse.saleInfo[lastIndex].ps - prev);
    } else {
      console.log("hatalı bilgi girdiniz");
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text) => (
        <span className="text-white block text-center text-lg sm:text-sm md:text-sm">
          {leftAmount ? Number(text) - Number(leftAmount) : text}
        </span>
      ),
    },
    {
