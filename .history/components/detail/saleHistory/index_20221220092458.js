...
export default function SaleHistory({ horse }) {
  ...
  for (let i = 0; i < horse.saleHistory.length; i++) {
    var seller = objToArr.filter(
      (seller) => seller.publicAddress === horse.saleHistory[i].sellerAddress
    );
    var buyer = objToArr.filter(
      (buyer) => buyer.publicAddress === horse.saleHistory[i].buyer
    );
    data.push({
      key: i,
      date: horse.saleHistory[i].date,
      seller: (
        <a target={"_blank"} href={`/owner/${seller.publicAddress}`}>
          {seller.username}
        </a>
      ),
      purchaser: (
