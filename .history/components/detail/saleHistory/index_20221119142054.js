...
export default function SaleHistory({ horse }) {
  ...
  for (let i = 0; i < horse.saleHistory.length; i++) {
    data.push({
      ...
      saler: <a target={"_blank"} href={`/owner/${horse.saleHistory[i].seller}`}>{horse.saleHistory[i].seller}</a>,
      purchaser: <a target={"_blank"} href={`/owner/${horse.saleHistory[i].buyer}`}>{horse.saleHistory[i].buyer}</a>,
      sellingPercent: `${horse?.saleHistory[i].ps / 100} %`,
      price: `${web3.utils.fromWei(horse.saleHistory[i].price,'ether')} ETH`,
    });
  }
  return (
          ...
          <>
            ...
            <div>
              <div className="flex items-center justify-between flex-wrap w-full text-sm 2xl:text-lg mb-2">
                <span className="mr-4">Owner</span> <span>%{shareholder?.percentage/100}</span>
              </div>
              <Link href="#">
                <a className="font-PoppinsSemiBold text-base 2xl:text-lg hover:text-white">
                  {`@${shareholder?.publicAddress.slice(0,5)}...${shareholder?.publicAddress.slice(-5)}`}
                </a>
              </Link>
            </div>
          </div>
