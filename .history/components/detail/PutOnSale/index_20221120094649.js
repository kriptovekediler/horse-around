...
export default function PutOnSale({ horse }) {
  ...
  const submit = async (e) => {
    ...
    await axios
      .post("https://horse-around-backend.herokuapp.com/put_on_sale", {
        publicAddress: account.data,
        token: getCookie("access_token"),
        horseId: horse.horseId,
        price: web3.utils.toWei(price.toString(), "ether").toString(),
        ps: sellingPercent,
        overseasBonus: horse.overseasBonus,
        signature: sign,
        nonce: nonce,
      })
      .then(setOnSale(true));
  ...
  const remove = async () => {
    ...
    await axios
      .post("https://horse-around-backend.herokuapp.com/remove_from_sale", {
        horseId: horse.horseId,
        publicAddress: account.data,
      })
