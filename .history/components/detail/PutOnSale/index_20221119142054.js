...
export default function PutOnSale( { horse } ) {
    ...
    const submit = async (e) => {
        ...
        await axios.post("https://horse-around-backend.herokuapp.com/put_on_sale", {
            publicAddress: account.data,
    ...
    const remove = async () => {
        await contractMarketplace?.methods.blockNonce(horse?.saleInfo?.nonce).send({
            from: account.data
       }).then(setOnSale(false))
        await axios.post("https://horse-around-backend.herokuapp.com/remove_from_sale", {
            horseId: horse.horseId,
            publicAddress: account.data,
                ...
                <div className="col-span-3">
                  ...
                  <form className="grid grid-cols-12 gap-4">
                    <div className="col-span-4">
                      <Input text="Price" placeholder="1 ETH"
                      onChange={(e) => setPrice(e.target.value)}
                      />
                    </div>
                    <div className="col-span-4">
                      <Input text="Selling Percent" placeholder="10 %"
