...
export default function BidHistory( { horse, account } ) {
  ...
  const acceptBid = async (tokenId, bidder, bidAmount) => {
    ...
    await contractAuction.methods.acceptBid(
      tokenId,
      bidder,
      web3.utils.toWei(bidAmount.toString(), "ether"),
      ).send({ from: account });

    await axios.post("https://horse-around-backend.herokuapp.com/accept_a_bid", {
      publicAddress: account,
      horseId: tokenId,
      buyerAddress: bidder,
      bidAmount: bidAmount,
  ...
  for (let i = 0; i < horse?.auctionInfo[horse?.auctionInfo.length-1].bidHistory.length; i++) {
      data.push({
        key: i,
        bindUser: {
          bidderAddress: horse?.auctionInfo[horse?.auctionInfo.length-1].bidHistory[i].bidderAddress,
        ...
        offer: (
            ...
            <span className={cn(styles.offerAmount,"mx-auto")}>
              {web3?.utils?.fromWei(horse?.auctionInfo[horse?.auctionInfo.length-1].bidHistory[i].bidAmount,'ether')} ETH
            </span>
          </div>
          ),
    });
