...
export default function EditPage({ horses }) {
  ...
  useEffect(() => {
    axios.post("https://horse-around-backend.herokuapp.com/get_user", {
      ...
      setOwned(res?.data.detail?.user?.myHorses.length);
      for(let i=0; i<res.data.detail?.user?.myHorses?.length; i++) {
        console.log("xyz",res.data.detail.user.myHorses[i])
        axios.post("https://horse-around-backend.herokuapp.com/get_horse", {
          horseId: res.data.detail.user.myHorses[i]
        }).then((res) => {
          console.log(res)
          if(res.data.detail.horse?.status === 3) {
            setOnSale(onSale + 1);
          }
        }
        )
          ...
          <div className="mt-10">
            ...
            <div className="grid grid-cols-12 gap-14">
              ...
              <div className="col-span-9 text-white">
                {activeTab === "Owned" && <HorseListBox horses={horses} count={owned} />}
                {activeTab === "On Sale" && <HorseListOnSale horses={horses} count={onSale} />}
                {activeTab === "My Auctions" && <MyAuctions horses={horses} account={account?.data} />}
                {activeTab === "Favorites" && <HorseListFavorites horses={horses} count={owned} />}
              </div>
            </div>
          </div>
        </Layout>
...
export async function getServerSideProps(context) {
  ...
  const response = await axios.post('https://horse-around-backend.herokuapp.com/verify', {
   ...
   if(response?.data?.status_code === 200){
        ...
        const res = await axios.get("https://horse-around-backend.herokuapp.com/get_horses");
        console.log("res",res)
        return {
          props: {
