...
export default function EditPage({ horses }) {
    ...
    axios
      ...
      .then((res) => {
        ...
        for (let i = 0; i < user?.myHorses?.length; i++) {
          console.log("xyz", user.myHorses[i]);
          console.log("status", user?.myHorses[i]?.status);
          setOwned(user?.myHorses?.length);
          if (user?.myHorses[i]?.status === 3) {
            setOnSale(onSale + 1);
          }
        }
      });
  ...
  console.log("aa", owned);
  console.log("Horse amount", user?.myHorses?.length);
          ...
          <div className="mt-10">
            ...
            <div className="grid grid-cols-12 gap-14">
              ...
              <div className="col-span-9 text-white">
                {activeTab === "Owned" && (
                  <HorseListBox horses={user?.myHorses} count={owned} />
                )}
                {activeTab === "On Sale" && (
