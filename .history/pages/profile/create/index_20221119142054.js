...
/*   useEffect(() => {
  ...
  const [horse, setHorse] = useState({
    ...
    horseName: "",
    ownerName: user?.user?.publicAddress.slice(0,5)+"..."+user?.user?.publicAddress.slice(-5),
    birthDate: "",
    age: "",
    sex: "",
    country: "",
...
/*     const reader = new window.FileReader();
  ...
  const mint = async (e) => {
    e.preventDefault();
      await axios.post("https://horse-around-backend.herokuapp.com/get_user",{
        publicAddress: account?.data
      }).then(res => {
        if(res.data.detail.user?.userType === "Creator"){
          console.log(horse)
          console.log(contractNFT)
          contractNFT.methods.mint("", 1000).send({
            ...
            web3.eth.getTransactionReceipt(res.transactionHash)
            .then(res => {
          ...
          alert("You are not a creator. Please contact the admin to get the creator role.")
        }
      })
  };
...
/*     formData.append("horseName", horse?.horseName);
    ...
    await axios.post("https://horse-around-backend.herokuapp.com/create_horse", {
      horse:horse,
      id: parseInt(id)
    })
/*     await axios({
      method: 'post',
      url: 'https://horse-around-backend.herokuapp.com/create_horse',
      data: formData,
      headers: {
          ...
          <div className="col-span-3">
            ...
            {step === 0 ? (
              ...
              <div className="col-span-4">
                <Input text="Horse Name" placeholder="John"
                value={horse.name}
                onChange={(e) => setHorse({ ...horse, horseName: e.target.value })}
                />
              </div>
              <div className="col-span-4">
                <Input text="Date of Birth" placeholder="16.04.2019"
                value={horse.birthDate}
                onChange={(e) => setHorse({ ...horse, birthDate: e.target.value })}
                />
              </div>
              <div className="col-span-2">
                <Input text="Age" placeholder="3 Y"
                value={horse.age}
                onChange={(e) => setHorse({ ...horse, age: e.target.value })}
                />
              </div>
              <div className="col-span-2">
                <Input text="Sex" placeholder="Female"
                value={horse.sex}
                onChange={(e) => setHorse({...horse,sex: e.target.value,})}
                 />
              </div>
              <div className="col-span-4">
                  <SelectBox text="Country" options={options} 
                  value={horse.country.label} onChange={changeHandler}
                  />
                </div>
              <div className="col-span-4">
                <Input text="Owner Name" placeholder={account?.data?.slice(0,5)+"..."+account?.data?.slice(-5)} disabled
                />
              </div>
              <div className="col-span-4">
                <Input text="Breeder Name" placeholder="Willhemm" 
                value={horse.breederName}
                onChange={(e) => setHorse({ ...horse, breederName: e.target.value })}
                />
              </div>
              <div className="col-span-4">
                <Input text="Jockey Name" placeholder="Willhemm"
                value={horse.jockeyName}
                onChange={(e) => setHorse({ ...horse, jockeyName: e.target.value })}
                />
              </div>
              <div className="col-span-4">
                <Input text="Sire Name" placeholder="Fire" 
                value={horse.sireName}
                onChange={(e) => setHorse({ ...horse, sireName: e.target.value })}
                />
              </div>
              <div className="col-span-4">
                <Input text="Dam Name" placeholder="Cardiac" 
                value={horse.damName}
                onChange={(e) => setHorse({ ...horse, damName: e.target.value })}
                />
              </div>
              <div className="col-span-4">
                <Input text="Dam-Siblings Name" placeholder="Cream, Simba" 
                value={horse.damSiblingsName}
                onChange={(e) => setHorse({ ...horse, damSiblingsName: e.target.value })}
                />
              </div>
              <div className="col-span-4">
                <Input text="Bonus" placeholder="12.000$" 
                value={horse.bonus}
                onChange={(e) => setHorse({ ...horse, bonus: e.target.value })}
                />
              </div>
              <div className="col-span-4">
                <Input text="Horse Owner Bonus" placeholder="400 $" 
                value={horse.horseOwnerBonus}
                onChange={(e) => setHorse({ ...horse, horseOwnerBonus: e.target.value })}
                />
              </div>
              <div className="col-span-4">
                <Input text="Breeding Bonus" placeholder="12.000 $" 
                value={horse.breedingBonus}
                onChange={(e) => setHorse({ ...horse, breedingBonus: e.target.value })}
                />
              </div>
              <div className="col-span-4">
                <Input text="Earning" placeholder="4.000 $" 
                value={horse.earning}
                onChange={(e) => setHorse({ ...horse, earning: e.target.value })}
                />
              </div>
              <div className="col-span-4">
                <Input text="Sponsorship Earnings" placeholder="2.000 $" 
                value={horse.sponsorshipEarnings}
                onChange={(e) => setHorse({ ...horse, sponsorshipEarnings: e.target.value })}
                />
              </div>
              <div className="col-span-4">
                <Input text="Overseas Bonus" placeholder="500 $" 
                value={horse.overseasBonus}
                onChange={(e) => setHorse({ ...horse, overseasBonus: e.target.value })}
                />
              </div>
              <div className="col-span-8">
                <Input text="Preference Description" placeholder="Description" 
                value={horse.preferenceDescription}
                onChange={(e) => setHorse({ ...horse, preferenceDescription: e.target.value })}
                />
              </div>              
                ...
                <div className="flex flex-row col-span-12 justify-end mr-12 mt-5 text-end gap-10">
                    <Button className="" onClick={(e) => setStep(0)}>Back Step</Button>
                    <div className="">
                      {loading ? (
                        <Button
                        onClick={() => alert("Please check the Metamask popup")}
                        >
                          <span className="flex">
                          <svg className="animate-spin h-5 mt-1 mr-2 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                          ...
                          Loading..
                            ...
                            <Button
                            className=""
                            onClick={(e) => {mint(e), setLoading(true)}}
                            >
                          <span>Mint</span>
                          </Button>
                        )}
...
export async function getServerSideProps(context) {
  ...
  const response = await axios.post('https://horse-around-backend.herokuapp.com/verify', {
      token: jwt,
      })
