...
export default function EditPage({ horses }) {
  ...
  useEffect(() => {
    axios
      .post("https://horsearound.herokuapp.com/get_user", {
      ...
      .then((res) => {
        ...
        for (let i = 0; i < res.data.detail?.user?.myHorses?.length; i++) {
          ...
          axios
            .post("https://horsearound.herokuapp.com/get_horse", {
          ...
          <div className="mt-10">
            ...
            <div className="grid grid-cols-12 gap-14">
              ...
              <div className="col-span-9 text-white">
                {activeTab === "Owned" && (
                  <HorseListBox horses={horses} count={owned} />
                )}
                {activeTab === "On Sale" && (
...
export async function getServerSideProps(context) {
  ...
  const response = await axios.post(
    "https://horsearound.herokuapp.com/verify",
    {
      token: jwt,
    }
  );
  ...
  if (response?.data?.status_code === 200) {
    ...
    const res = await axios.get("https://horsearound.herokuapp.com/get_horses");
    console.log("res", res);
    return {
      props: {
