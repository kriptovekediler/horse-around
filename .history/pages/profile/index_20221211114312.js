...
export default function EditPage({ horses }) {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      const filterUser = Object.entries(response?.detail).find(
        ([key, value]) => value?.publicAddress === account?.data
      );
      console.log("Vsas", filterUser[1]);
      setUser(filterUser[1]);
      // setUser(filterUser);
      setOwned(response?.detail?.user?.myHorses?.length);
    };
    getUser();
  }, [filterUser]);
