...
export default function MarketPlace({horse, status}) {
  const tabs = (status === 4 ? [
    "Bid History",
    "Properties",
    "Pedigree",
  ...
  const [activeTab, setActiveTab] = useState(horse?.status === 4 ? "Bid History" : horse?.status === 2 ? "Offer History" : horse?.status === 3 ? "Shares on Sale" : "Properties");
      ...
      <div className="home">
        ...
        <Layout page>
          ...
          <div className="">
            ...
            {activeTab === "Bid History" && status === 4 && <BidHistory horse={horse} account={account.data} />}
            {activeTab === "Offer History" && status === 2 && <OfferHistory horse={horse} account={account.data} />}
            {activeTab === "Shares on Sale" && status === 3 && <SharesOnSale horse={horse} account={account.data} />}
...
export async function getServerSideProps(context) {
  ...
  console.log("ids", id);
  const res = await axios.post("https://horse-around-backend.herokuapp.com/get_horse", {
    horseId: parseInt(id), 
  });
