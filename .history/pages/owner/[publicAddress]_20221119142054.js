              ...
              <div className="grid grid-cols-3 md:grid-cols-2 sm:grid-cols-1  gap-4 mt-10">
                {activeTab === "All Sales" && (
                  Object.entries(horses?.detail).map(([key, value]) => {
                    if((value.status === 3 || value.status === 4) && value.publicAddress === user?.detail.user?.publicAddress){
                      return (
                        ...
                        <a>
                          <HorseCard
                            ...
                            price={value?.status === 3 ? value?.saleInfo.price : value?.status === 4 ? value?.auctionInfo[value?.auctionInfo.length-1].reservedPrice : 0}
                            id={value.horseId}
                            sex={value.sex}
                            age={value.age}
                            web3={web3}
                ...
                {activeTab === "Past Sales" && (
                  Object.entries(horses?.detail).map(([key, value]) => {
                    if(user?.detail.user?.soldHorses?.includes(value.horseId)){
                      return (
                        <Link href={`/detail/${value.horseId}`} key="1">
                        <a>
                          <HorseCard
...
export async function getServerSideProps(context) {
  ...
  const res = await axios.post("https://horse-around-backend.herokuapp.com/get_user", {
  ...
  const resp = await axios.get("https://horse-around-backend.herokuapp.com/get_horses");
  return {
    props: {
      user: res.data,
      horses: resp.data
